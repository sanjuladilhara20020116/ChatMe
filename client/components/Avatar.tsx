...
export default function Avatar({name,size = 40,online,src}:
AvatarProps) {
  ...
  const indicatorSize = Math.round(size * 0.38)

  return (
    <View style={[styles.root, {width: size, height: size, borderRadius: size/2}]}>
      <View style={[styles.root, {width: size, height: size, 
        borderRadius: size/2, backgroundColor: src ? "transparent": color}]}>
          {src ?(
